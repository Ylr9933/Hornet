...
JumpState::JumpState(QString name, QMovie *movie,Widget *widget,StateMachine *stateMachine)
    :State(name,movie,widget,stateMachine)
{
    connect(widget,&Widget::mouseEvent,this,&JumpState::onMouseEvent);
    ...
    animation->setDuration(250); // 动画持续时间为100毫秒
...
void JumpState::enter()
{
    ...
    animation->setEndValue(widget->pos()+QPoint(-50,-100));
    delayTimer->start(400); // 延迟975毫秒
    State::enter();
}

void JumpState::onMouseEvent(QMouseEvent *event, QPoint mousePoint,  int eventType)
{
    //如果状态机不处于本状态则不管
